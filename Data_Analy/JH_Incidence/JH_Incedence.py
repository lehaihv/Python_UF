...
covid_data = pd.read_csv("covid19_confirmed_US.csv")
# print(covid_data.head())
# print(covid_data.iloc[0])
# filename = covid_data.Admin2[0] + ".csv"
# filename = "JH_Incidence/" + covid_data.Admin2[0] + ".xlsx"
filename = "1.csv"
for x in range(0, 500):  # 3342

    covid_data.iloc[x].to_csv("temp1.csv", header=None)

    # Step 2: Read temp1, prepare data to calculate R, add 'confirmed_by_pop' column and save to temp2
    covid_data_temp = pd.read_csv("temp1.csv", names=['dates', 'I', 'confirmed_by_pop'])
    # print(covid_data.head())
    covid_data_temp.to_csv("temp2.csv", index=False)

    # Step 3: Read temp2, format dates, calculate deaths by pop, save to regions files
    covid_data_temp = pd.read_csv("temp2.csv")
